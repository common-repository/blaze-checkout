 ...
 * Action Add
 */
add_action( 'manage_posts_extra_tablenav', 'blaze_payment_orders_export_button', 20, 1 );
add_action( 'init', 'blaze_payment_orders_export' );
	
/***
 * Place order button place Export
 * 
 */
function blaze_payment_orders_export_button( $which ) {
	global $typenow;
 ...
 * Show Error .
 */
function blaze_error()
{
	echo '<div class="error">';
	echo "<p>" .__("Date is not valid.", "blazeCheckout" ) . "</p>";
	echo "</div>";
}
 ...
 * 
 */
function blaze_payment_orders_export(){
	if(isset($_GET['export_all_posts'])) {
		...
		if(is_array($validation) && $validation['errors'] ){
			add_action("admin_notices", "blaze_error");
			return;
		}
		...
		header('Content-Disposition: attachment; filename="blaze-order.csv"');
		header('Pragma: no-cache');
		header('Expires: 0');

		$file = fopen('php://output', 'w');

		fputcsv($file, array('Order Id', 'Order Date', 'Order Status', 'Transaction ID','Payment Method' ,'Item Sub Total','Voucher Amount','Blaze Discount','Shipping Total','Paid Amount','Refund Amount','Tags','Customer Email'));
		...
		if ( $loop->have_posts() ): 
			...
			$paymentMode = $blazecheckoutId = "";
			$blazeDiscount  = get_post_meta( $order_id, 'blaze_discount_amount', true );
			$paidAmount = $order->get_total();
			if($blazeMode === 'UPI'){
				$blazecheckoutId = get_post_meta( $order_id, 'blaze_payment_id', true );
				$paymentMode = "Blaze UPI";
				if(is_numeric($blazeDiscount) && $blazeDiscount != ''){
					$paidAmount = $order->get_total() - $blazeDiscount; 
				}
			}else if ($blazeMode === 'COD') {
				$paymentMode = "Blaze COD";
			}
			$tags = get_post_meta( $order_id, 'blaze_tags', true );
			$couponAmount = 0;
			...
			// $line_subtotal_tax = $order->get_subtotal_tax();
			// $line_total_tax    = $order->get_total_tax();
